...
class DayPlanner:
    ...
    def add_flip_card_side_id_to_day_plan(self, side: FlipCardSideId, days: NumberOfDays):
        ...
        if not day_plan:
            day_plan = self.factory.create_day_plan(
                unique_flip_card_sides_container=UniqueFlipCardSidesContainer({side}), day=day)
        else:
            day_plan.add_to_day_plan(side=side)

        self.repository.purge_side_id_from_all_plans_and_save_plan(side_id_to_purge=side, day_plan=day_plan)
